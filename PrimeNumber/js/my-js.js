...
function checkifPrimeNumber(n) {
    ...
    if(n < 2) //Số nguyên tố nhỏ nhất là 2. Kiểm tra nếu n < 2 thì không thể là số nguyên tố, trả về false
        return false;
    for (let i=2; i <= n-1; i++) //Lặp từ 2 tới (n-1), nếu trong khoảng này tồn tại số mà n chia hết thì kết luận n không phải là số nguyên tố, ngược lại n là số nguyên tố.
    if(n % i === 0)
        return false;
    return true;
}